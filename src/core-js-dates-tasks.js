...
function getNextFriday(date) {
  ...
  const nextFridayMs = date.getTime() + daysToNextFriday * 24 * 60 * 60 * 1000;

  return new Date(nextFridayMs);
}
...
function getCountDaysInMonth(month, year) {
  return new Date(year, month).toUTCString().split(' ')[1];
}
...
function formatDate(date) {
  ...
  const day = dateWithOffset.getDate();
  const month = dateWithOffset.getMonth() + 1;
  const year = dateWithOffset.getFullYear();
  const hours24 = dateWithOffset.getHours() + 24;
  const hours12 = dateWithOffset > 12 ? hours24 % 12 : hours24 - 24;
  const minutes = dateWithOffset.getMinutes().toString().padStart(2, '0');
  const seconds = dateWithOffset.getSeconds().toString().padStart(2, '0');

  return `${month}/${day}/${year}, ${
    hours12 === 0 ? 12 : hours12
  }:${minutes}:${seconds} ${hours24 - 24 >= 12 ? 'PM' : 'AM'}`;
}
